...
**Eval Protocol is like a universal adapter for AI evaluation — it allows reward functions to work seamlessly across any platform, infrastructure, or training pipeline.**

Just as USB-C provides a universal standard for device connectivity, Eval Protocol establishes a universal standard for AI model evaluation, enabling reward functions to be written once and deployed anywhere.
...
Eval Protocol solves these problems by providing **a single, standardized way to define reward functions that work everywhere**.

## How Eval Protocol Works

```mermaid
graph TB
    A[Your Reward Function] --> B[Eval Protocol]
...
Eval Protocol uses a **client-server model** where reward functions act as specialized evaluation servers that can be called by various AI training and evaluation clients.
