...
[TRL (Transformer Reinforcement Learning)](https://huggingface.co/docs/trl) is a popular library for fine-tuning language models using reinforcement learning. Eval Protocol provides seamless integration with TRL, allowing you to use your evaluation functions directly in training pipelines.

## Overview

The TRL integration enables you to:
- Use Eval Protocol evaluation functions as reward models in TRL training
...
- Monitor evaluation function performance during training
...
### Using Evaluation Functions in PPO Training
...
reward_model = RewardProtocolRewardModel(
    evaluation_function=conversation_quality_reward,
    device="cuda",  # or "cpu"
    batch_size=8
)
...
from eval_protocol.integrations.trl import create_grpo_evaluation_function

# Create GRPO-compatible evaluation function
@evaluation_function
...
grpo_trainer = GRPOTrainer(
    ...
    evaluation_function=create_grpo_evaluation_function(math_accuracy_reward),
)
...
Combine multiple evaluation functions for complex training objectives:
...
reward_model = RewardProtocolRewardModel(
    ...
    compile_function=True,   # JIT compile evaluation function
)
...
# Create reward model
reward_model = RewardProtocolRewardModel(
    evaluation_function=sentiment_reward,
...
reward_model = RewardProtocolRewardModel(
    evaluation_function=your_function,
...
```python
# Add error handling to evaluation function
@evaluation_function
...
1. **Start Simple**: Begin with basic evaluation functions before adding complexity
