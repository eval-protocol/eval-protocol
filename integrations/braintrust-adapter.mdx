...
adapter = BraintrustAdapter(
    api_key="your_api_key",        # Or set BRAINTRUST_API_KEY env var
    project_id="your_project_id"   # Or set BRAINTRUST_PROJECT_ID env var
)

# Pull recent evaluation rows using BTQL
btql_query = """
select: *
...
async def test_braintrust_data(rows: list[EvaluationRow]) -> list[EvaluationRow]:
    return await aha_judge(rows, adapter=adapter)
```
