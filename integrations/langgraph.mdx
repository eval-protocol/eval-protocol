...
To supply a LangGraph app for evaluation, you define a factory function that
accepts a config dict and returns a compiled graph with `.ainvoke`.
...
Use `completion_params` to get the model name and parameters, then
construct your LangGraph app accordingly.
...
Use `completion_params` and `build_graph_kwargs` to pass `reasoning_effort` values like
"low", "medium", or "high".
