...
`eval-protocol` easily works with Pydantic AI agents.
...
We are using the `completion_params` to get the model name and provider, and
then creating a Pydantic AI model using the `OpenAIModel` class.
