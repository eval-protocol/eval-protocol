...
Use the `completion_params` to get the model name and provider, and then
creating a Pydantic AI model using the `OpenAIModel` class.
...
1. **Agent Factory**: The `agent_factory` function creates a Pydantic AI agent using the model and provider from `completion_params`
