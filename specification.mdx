...
The `EvaluationTest` represents a test configuration for evaluating models. While not explicitly defined as a separate class in the current implementation, evaluation tests are typically configured through:

- **Dataset Configuration**: JSONL files containing test cases
- **Model Configuration**: Model parameters and completion settings
- **Evaluation Criteria**: Success thresholds and metrics
