...
description: 'Quickly test reward functions with sample data before deployment'
---

# reward-protocol preview

The `preview` command allows you to quickly test reward functions with sample data, making it ideal for development, debugging, and validation before deploying to production.
...
#### --metrics-folders
Specify reward functions to test using the format `name=path`.
...
# Custom parameters passed to reward functions
custom_params:
...
```bash
Error: Reward function 'my_function' not found in ./metrics/my_metric
```
...
```bash
Error: Failed to import reward function: ModuleNotFoundError
```
