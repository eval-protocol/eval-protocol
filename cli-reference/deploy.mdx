...
description: 'Deploy reward functions to production environments'
---

# reward-protocol deploy

The `deploy` command packages and deploys your reward functions to production environments, making them available for use in training pipelines, evaluation systems, and other applications.
...
#### --metrics-folders
Specify reward functions to deploy using the format `name=path`.
...
reward-protocol deploy \
  --id simple-eval \
  --function-ref "rewards.simple.basic_reward_function"
```
...
```bash
Error: Reward function not found in ./nonexistent/path
```
