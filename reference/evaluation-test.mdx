...
description: "Create pytest-based evaluation tests for AI model evaluation with support for pointwise, groupwise, and all modes"
---
...
Generation parameters for the rollout. The required fields depend on the rollout processor used:
...
- Must include `model` and `provider` fields
- Example: `{"model": "accounts/fireworks/models/kimi-k2-instruct", "provider": "fireworks"}`
- The `provider` field is used to create the appropriate Pydantic AI model
...
<ParamField path="input_rows" type="Optional[List[EvaluationRow]]">
Pre-constructed EvaluationRow objects to use directly. Useful when you already have messages and/or metadata prepared. Will be passed as "input_dataset" to the test function.
</ParamField>
...
- Parameter combinations are generated across `input_dataset`, `completion_params`, `input_messages`, `input_rows`, and `evaluation_test_kwargs`.
- Pytest parameter names (in order when present): `dataset_path`, `completion_params`, `input_messages`, `input_rows`, `evaluation_test_kwargs`.
...
<ParamField path="completion_params" type="CompletionParams" required>
...
- Must include both `model` and `provider` fields
...
<ParamField path="mcp_config_path" type="str" required>
...
<ParamField path="semaphore" type="asyncio.Semaphore" required>
...
<ParamField path="logger" type="DatasetLogger">
