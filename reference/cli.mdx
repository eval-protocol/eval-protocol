...
<ParamField path="--loss-config-method" type="Literal">
  RL loss method for underlying trainers. One of {grpo,dapo}.
</ParamField>
...
<ParamField path="--training-config-gradient-accumulation-steps" type="number">
  The number of batches to accumulate gradients before updating the model parameters. The effective batch size will be batch-size multiplied by this value.
</ParamField>
...
<ParamField path="--training-config-learning-rate-warmup-steps" type="number">
  The number of learning rate warmup steps for the reinforcement fine-tuning job.
</ParamField>
