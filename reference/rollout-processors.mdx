...
- **What it does**: Runs Pydantic AI agents. Supports both single agent and multi-agent scenarios.
- **When to use**: **ONLY for Pydantic AI framework.**
- **Requires**: `kwargs` must contain an `agent` field with either a Pydantic AI Agent instance or a function that returns one.
...
Usage with dynamic agent creation for multi-agent scenarios:
...
def test_pydantic_multi_agent(row: EvaluationRow) -> EvaluationRow:
    return row
```
