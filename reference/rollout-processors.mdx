...
def agent_factory(config: RolloutProcessorConfig) -> Agent:
    ...
    joke_selection_model = OpenAIModel(
...
- **What it does**: Delegates rollout execution to a remote HTTP service that you control. Useful for offloading multi-turn agent loops, RL-style interactions, or using a centralized inference gateway.
- **When to use**: You want to scale rollouts outside the pytest process, or integrate with existing batch/serving infrastructure.
...
- GET `/status?rollout_id=rll_ijkl`
  - Response: `{ "terminated": true, "info": { ... } }`
...
@evaluation_test(
    ...
    completion_params=[{"model": "openai/gpt-4o"}],
    rollout_processor=RemoteRolloutProcessor(remote_base_url="https://my-remote-evaluator.example.com", num_turns=2),
    mode="pointwise",
)
