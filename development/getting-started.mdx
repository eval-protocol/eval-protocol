...
This guide covers advanced development patterns and best practices for building robust evaluation functions and integrating them into production systems.
...
class TestAdvancedEvaluator(EvaluationFunctionTest):
...
# Copy function code
COPY evaluation_functions/ ./evaluation_functions/
...
HEALTHCHECK --interval=30s --timeout=10s --retries=3 \
  CMD python -c "from evaluation_functions import health_check; health_check()"
...
kind: Deployment
metadata:
  name: evaluation-function-deployment
spec:
  replicas: 3
  selector:
    matchLabels:
      app: evaluation-function
  template:
    metadata:
      labels:
        app: evaluation-function
    spec:
      ...
      - name: evaluation-function
        image: your-registry/evaluation-function:latest
        ports:
        - containerPort: 8000
        env:
