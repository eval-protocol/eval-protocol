...
reward_fn = load_function("your_evaluation_function")
...
trainer = PPOTrainer(
    ...
    tokenizer=tokenizer,
    dataset=dataset,
    reward_fn=reward_fn,  # Use your evaluation function
)

# Train with evaluation function
trainer.train()
```
...
class MyEvaluationPipeline:
    def __init__(self):
        ...
        self.evaluation_functions = self.protocol.load_functions([
            "math_accuracy",
            "code_quality",
            "factual_correctness"
        ])
    ...
    def evaluate_batch(self, queries, responses):
        ...
        for query, response in zip(queries, responses):
            scores = {}
            for name, fn in self.evaluation_functions.items():
...
Deploy evaluation functions as REST endpoints:
