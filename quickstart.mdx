...
**Not bad!** Note that the percentages don't mean the same thing as the actual scores. Our percentages represent **win rates in pairwise comparisons** against the original model (kimi-k2), while Tau-Bench scores are absolute performance metrics.
...
**This ranking matches our real Tau-Bench airline scores!** Pretty amazing we're able to achieve this validation without actually running against the Tau-Bench benchmark - just using production conversation traces and an LLM judge.
