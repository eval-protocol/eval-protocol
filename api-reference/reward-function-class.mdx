...
description: 'Advanced reward function management and deployment using the RewardFunction class'
---

# RewardFunction Class

The `RewardFunction` class provides an advanced interface for creating, managing, and deploying reward functions programmatically. While the `@reward_function` decorator is the recommended approach for most use cases, the class interface offers additional control and features.
...
# Create RewardFunction instance
...
### name: str
A unique identifier for your reward function.
...
Human-readable description of what the reward function evaluates.

### metadata: Dict[str, Any] (optional)
Additional metadata about the reward function.
...
Execute the reward function with provided inputs.
...
Deploy the reward function to a target platform.
...
if validation_result.is_valid:
    print("Reward function is valid")
else:
...
Add custom validation logic for your reward function:
...
Save and load reward function configurations:
...
# Load configuration
loaded_func = RewardFunction.load_config("./my_evaluator_config.json")
...
async_reward_func = RewardFunction(
...
1. **Use descriptive names**: Choose clear, unique names for your reward functions
